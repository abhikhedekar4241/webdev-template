...
def test_log_event_creates_record(session: Session):
    user_id = uuid.uuid4()
...
def test_log_event_with_org_and_metadata(session: Session):
    org_id = uuid.uuid4()
    log_event(
        session,
        event="org.created",
...
def test_log_event_all_optional(session: Session):
...
def test_log_event_success(session: Session):
    user_id = uuid.uuid4()
    log_event(session, event="test_event", user_id=user_id)
    
    entry = session.exec(select(AuditLog).where(AuditLog.event == "test_event")).first()
...
def test_log_event_failure():
    ...
    log_event(mock_session, event="failed_event")
