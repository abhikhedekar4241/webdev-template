...
async def test_authenticate_user_not_found(session):
    assert await auth_service.authenticate(session, email="not@found.com", password="p") is None
...
async def test_authenticate_oauth_only(session):
    ...
    assert await auth_service.authenticate(session, email="oauth@only.com", password="p") is None

async def test_authenticate_wrong_password(session, alice):
    # alice is created by fixture with password 'password'
    assert await auth_service.authenticate(session, email=alice.email, password="wrong") is None
