...
async def test_create_org_service(session: Session, alice):
    org = await org_service.create_org(session, name="Acme", slug="acme", created_by=alice.id)
    assert org.name == "Acme"
    assert org.slug == "acme"
    membership = await org_service.get_membership(session, org_id=org.id, user_id=alice.id)
    assert membership is not None
    assert membership.role == OrgRole.owner


async def test_list_user_orgs(session: Session, alice, bob):
    org1 = await org_service.create_org(session, name="Org1", slug="org1", created_by=alice.id)
...
async def test_get_org_returns_none_if_not_member(session: Session, alice, bob):
    ...
    result = await org_service.get_org_for_member(session, org_id=org.id, user_id=bob.id)
...
async def test_get_org_returns_org_for_member_service(session: Session, alice):
    ...
    result = await org_service.get_org_for_member(session, org_id=org.id, user_id=alice.id)
...
async def test_update_org_service(session: Session, alice):
    ...
    updated = await org_service.update_org(session, org=org, name="New", slug="new-slug")
...
async def test_change_role_service(session: Session, alice, bob):
    org = await org_service.create_org(session, name="Team", slug="team2", created_by=alice.id)
    await org_service.add_member(
        session, org_id=org.id, user_id=bob.id, role=OrgRole.member
    )
...
async def test_remove_member_service(session: Session, alice, bob):
    org = await org_service.create_org(session, name="Team", slug="team3", created_by=alice.id)
    await org_service.add_member(
        session, org_id=org.id, user_id=bob.id, role=OrgRole.member
    )
