...
async def test_onboarding_with_org_creation(
    client: AsyncClient, alice, session: AsyncSession
):
    ...
    org = (
        await session.exec(select(Organization).where(Organization.name == "New Org"))
    ).first()
    assert org is not None
    assert org.created_by == alice.id


async def test_onboarding_with_existing_slug_collision(
    client: AsyncClient, alice, alice_org, session: AsyncSession
):
    ...
    org = (
        await session.exec(
            select(Organization).where(Organization.name == "Colliding Slug")
        )
    ).first()
    assert org is not None
...
async def test_google_callback_state_mismatch(client: AsyncClient):
    ...
    resp = await client.get(
        "/api/v1/auth/google/callback?state=wrong-state", follow_redirects=False
    )
    assert resp.status_code == 307
...
async def test_google_callback_oauth_error(client: AsyncClient):
    ...
    with patch(
        "app.api.v1.auth.oauth_service.exchange_code",
        side_effect=httpx.HTTPError("fail"),
    ):
        resp = await client.get(
            "/api/v1/auth/google/callback?state=state123&code=c", follow_redirects=False
        )
        assert resp.status_code == 307
        assert "error=oauth_failed" in resp.headers["location"]
...
async def test_register_already_verified(client: AsyncClient, session):
    ...
    resp = await client.post(
        ...
        json={"email": "v@f.com", "password": "password", "full_name": "V"},
    )
...
async def test_resend_too_soon(client: AsyncClient, alice, session):
    ...
    resp = await client.post(
        "/api/v1/auth/resend-verification", json={"email": alice.email}
    )
