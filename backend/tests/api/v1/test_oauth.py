...
async def test_google_callback_creates_new_user(client: TestClient, session: Session):
    client.cookies.set("oauth_state", "state123")
    with patch(
        "app.api.v1.auth.oauth_service.exchange_code", return_value=_google_token()
    ):
        with patch(
...
async def test_google_callback_uses_existing_oauth_account(
    client: TestClient, session: Session
):
...
async def test_google_callback_inactive_user_redirects_to_error(
    client: TestClient, session: Session
):
    user = await auth_service.create_user(
        session,
