...
async def test_list_orgs_returns_only_user_orgs_api(
    client: TestClient, alice, bob, session: Session
):
    await org_service.create_org(
        session, name="Alice Org", slug="alice-org", created_by=alice.id
    )
    await org_service.create_org(
        session, name="Bob Org", slug="bob-org", created_by=bob.id
    )
...
async def test_get_org_as_non_member_returns_404(
    client: TestClient, alice, bob, session: Session
):
    org = await org_service.create_org(
...
async def test_update_org_as_member_returns_403(
    client: TestClient, alice, bob, session: Session
):
    org = await org_service.create_org(
        session, name="Org", slug="org", created_by=alice.id
    )
...
async def test_delete_org_as_owner_api(client: TestClient, alice, session: Session):
    ...
    resp = await client.delete(
        f"/api/v1/orgs/{org.id}", headers=get_auth_header(alice.id)
    )
...
async def test_delete_org_as_admin_returns_403(
    ...
    org = await org_service.create_org(
        session, name="Org", slug="org2", created_by=alice.id
    )
    await org_service.add_member(
        session, org_id=org.id, user_id=bob.id, role=OrgRole.admin
    )
    resp = await client.delete(
        f"/api/v1/orgs/{org.id}", headers=get_auth_header(bob.id)
    )
    assert resp.status_code == 403
...
async def test_list_members_api(client: TestClient, alice, bob, session: Session):
    ...
    resp = await client.get(
        f"/api/v1/orgs/{org.id}/members", headers=get_auth_header(alice.id)
    )
    assert resp.status_code == 200
