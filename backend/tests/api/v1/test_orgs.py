...
def test_create_org_api(client: TestClient, alice):
    resp = client.post(
        "/api/v1/orgs/",
...
def test_list_orgs_returns_only_user_orgs_api(client: TestClient, alice, bob, session: Session):
    ...
    resp = client.get("/api/v1/orgs/", headers=get_auth_header(alice.id))
...
def test_get_org_as_member_api(client: TestClient, alice, session: Session):
    org = org_service.create_org(session, name="Mine", slug="mine", created_by=alice.id)
    resp = client.get(f"/api/v1/orgs/{org.id}", headers=get_auth_header(alice.id))
    assert resp.status_code == 200
    assert resp.json()["id"] == str(org.id)


def test_get_org_as_non_member_returns_404(client: TestClient, alice, bob, session: Session):
    org = org_service.create_org(
        session, name="Private", slug="private", created_by=alice.id
    )
    resp = client.get(f"/api/v1/orgs/{org.id}", headers=get_auth_header(bob.id))
    assert resp.status_code == 404


def test_update_org_as_owner_api(client: TestClient, alice, session: Session):
...
def test_update_org_as_member_returns_403(client: TestClient, alice, bob, session: Session):
...
def test_delete_org_as_owner_api(client: TestClient, alice, session: Session):
    org = org_service.create_org(
        session, name="ToDelete", slug="to-delete", created_by=alice.id
    )
    resp = client.delete(f"/api/v1/orgs/{org.id}", headers=get_auth_header(alice.id))
    assert resp.status_code == 204


def test_delete_org_as_admin_returns_403(client: TestClient, alice, bob, session: Session):
    ...
    resp = client.delete(f"/api/v1/orgs/{org.id}", headers=get_auth_header(bob.id))
    assert resp.status_code == 403


def test_list_members_api(client: TestClient, alice, bob, session: Session):
    org = org_service.create_org(session, name="Team", slug="team", created_by=alice.id)
    org_service.add_member(session, org_id=org.id, user_id=bob.id, role=OrgRole.member)
    resp = client.get(f"/api/v1/orgs/{org.id}/members", headers=get_auth_header(alice.id))
    assert resp.status_code == 200
    assert len(resp.json()) == 2


def test_change_member_role_api(client: TestClient, alice, bob, session: Session):
    org = org_service.create_org(
...
def test_remove_member_api(client: TestClient, alice, bob, session: Session):
    org = org_service.create_org(
...
def test_cannot_remove_owner_api(client: TestClient, alice, session: Session):
    org = org_service.create_org(
