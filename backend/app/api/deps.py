...
async def get_current_user(
    ...
    if token.startswith(_API_KEY_PREFIX):
        key_record = await api_key_service.authenticate(session, raw_key=token)
        if not key_record:
            raise HTTPException(status_code=401, detail="Invalid or revoked API key")
        user = await session.get(User, key_record.created_by)
...
async def require_org(session: AsyncSession, org_id: uuid.UUID, user_id: uuid.UUID):
    """Return org if user is a member, else raise 404."""
    org = await org_service.get_org_for_member(session, org_id=org_id, user_id=user_id)
    if not org:
